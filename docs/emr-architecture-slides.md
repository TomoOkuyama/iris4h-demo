...
| データベース | IRIS for Health（JsonAdvSQL） | FHIR リポジトリ（SQL 分析はプロジェクション経由） |
...
5. **同じデータに SQL でもアクセス可能** — プロジェクション定義で BI ツール連携（別途 Vector Search も利用可能）
